...
export class ItemComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  get sameThemeItems (): Array<Item> {
    const sameThemeItems = this.cpntData.items.all.filter(item => {
  ...
  generateLaonetics () {
    ...
    if (lo.wrd) {
      const slicedSyllables: ISlicedSyllables = this.translater.getKaraoke(lo.wrd, [
        'fr', 'en', 'ph'
      ]);
    ...
    if (meta && meta.conflicts && meta.conflicts.length) {
      this.cpntData.selectedThemes.forEach(theme => {
        meta.conflicts.forEach((conflictedItem, idx) => {
          const otherItem = this.cpntData.items.all.find(item => {
